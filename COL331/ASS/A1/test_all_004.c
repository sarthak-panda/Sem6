...
void run_test(int i, int total_processes, struct shared_data *shared) {
    pid_t pid = getpid();
    int ret;
    ...
    TEST_SYSCALL(syscall(SYS_RESOURCE_CAP, pid, 9L, 6L),
    ...
    void *malloc_mem_extra = allocate_mb(2*(5-i));
    ...
    while (shared->count < total_processes) {
        sleep(1);
    }

    printf("PID %d: List of all test process PIDs: ", pid);
    for (int j = 0; j < total_processes; j++) {
        printf("%d ", shared->pids[j]);
    }
    printf("\n");
}
...
int main(void) {
    ...
    struct shared_data *shared = mmap(NULL,
        sizeof(struct shared_data) + num_processes * sizeof(pid_t),
    ...
    for (int i = 0; i < num_processes; i++) {
        ...
        if (pid < 0) {
            ...
            run_test(i, num_processes, shared);
            exit(EXIT_SUCCESS);
        } else {
