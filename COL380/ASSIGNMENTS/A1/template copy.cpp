...
vector<float> matmul_multiply(const map<pair<int, int>, vector<vector<int>>>& blocks_dash,map<pair<int, int>, vector<vector<int>>>& blocks, int n, int m, bool stats_needed) {
    ...
    // Preallocate vectors
    keys1.resize(blocks_dash.size());
    keys2.resize(blocks.size());

    {
        int index = 0;
        // First, create a vector of iterators for blocks_dash
        std::vector<decltype(blocks_dash.begin())> iters_dash;
        for (auto it = blocks_dash.begin(); it != blocks_dash.end(); ++it) {
            iters_dash.push_back(it);
        }
        // Similarly for blocks
        std::vector<decltype(blocks.begin())> iters;
        ...
        #pragma omp parallel
        {
            ...
            {
                // Use taskloop to fill keys1 in parallel.
                #pragma omp taskloop
                for (int i = 0; i < iters_dash.size(); ++i) {
                    keys1[i] = iters_dash[i]->first;
                }
                
                // Use taskloop to fill keys2 in parallel.
                #pragma omp taskloop
                for (int i = 0; i < iters.size(); ++i) {
                    keys2[i] = iters[i]->first;
                }
            }
        }
    }
    ...
    #pragma omp parallel if(black_box())
    {
        #pragma omp single
        {
            #pragma omp taskloop collapse(2) if(black_box())
            for (size_t i_loop = 0; i_loop < n1; i_loop++) {
                for (size_t j_loop = 0; j_loop < n2; j_loop++) {
                    ...
                    #pragma omp taskloop collapse(2) shared(temp_result, P) firstprivate(i, stats_needed, entry, entry2, m)
                        for (int x = 0; x < m; x++) {
                            for (int y = 0; y < m; y++) {
                                int sum = 0;
                                for (int z = 0; z < m; z++) {
