...
def plot(data_plot,sizes):
    ...
    for i in range(6):
        y_axis_data[i]=[]
        for size in sizes:
            y_axis_data.append(data_plot[size][i][0])
    for i in range(6):
        plt.plot(sizes,y_axis_data[i],label=type_map[i])
    plt.legend()
    plt.savefig("user time vs matrix multiplication size.png", dpi=400, bbox_inches="tight")
    plt.show()
    ...
    for i in range(6):
        y_axis_data[i]=[]
        for size in sizes:
            y_axis_data.append(data_plot[size][i][1])
    for i in range(6):
        plt.plot(sizes,y_axis_data[i],label=type_map[i])
    plt.legend()
    plt.savefig("user time in matrix multiplication vs matrix multiplication size.png", dpi=400, bbox_inches="tight")
    plt.show()    
    ...
    for i in range(6):
        y_axis_data[i]=[]
        for size in sizes:
            y_axis_data.append(data_plot[size][i][2])
    for i in range(6):
        plt.plot(sizes,y_axis_data[i],label=type_map[i])
    plt.legend()
    plt.savefig("cache hit rate vs matrix multiplication size.png", dpi=400, bbox_inches="tight")
    plt.show()       
def helper(perm_num,size,data_plot):
    ...
    for idx,line in enumerate(lines):
        ...
        if task_clock_phase:
            ...
            if data_to_collect[data_idx] in line:
                ...
                data_idx+=1
    data_plot[size][perm_num]=[data_map['task-clock']/(10**9),(data_map['task-clock-matrix-multiply']*data_map['task-clock'])/(100*(10**9)),1-(data_map['cpu_core/cache-misses/']/data_map['cpu_core/cache-references/'])]
    ...
    row_labels=['%age of time spent in matrixMultiply','%age of time spent in readMatrix','%age of time spent in writeMatrix','%age of time spent in main']
    table_data = {
        "gprof": [data_map['gprof-matrix-multiply'],data_map['gprof-read-matrix'],data_map['gprof-write-matrix'],'N/A'],
        "perf": [data_map['task-clock-matrix-multiply'],data_map['task-clock-read-matrix'],data_map['task-clock-write-matrix'],data_map['task-clock-main']]
    }
    ...
    \\end{{document}}
    """
    with open(f'perf_gprof_table_{size}.tex','w') as f:
...
for size in range(1000,6000,1000):
    ...
    for perm_num in range(6):
        path_input, path_output = f"./input_path_{size}/", f"./output_path_{size}/"
        ...
        os.system(f"perf record -e task-clock,cycles,instructions,cache-references,cache-misses,user_time -F 1000 -g --no-kernel ./main 0 {size} {size} {size} ./input_path_{size}/ ./output_path_{size}/")
        os.system("perf report --stdio > perf_report.txt")
        os.system("gprof ./main gmon.out > analysis.txt")
