...
vector<vector<vector<int>>> modify(vector<vector<vector<int>>>& matrices, vector<int>& range) {
    ...
    try {
        ...
        checkCuda(cudaMemcpyAsync(d_range, range.data(), numMatrices * sizeof(int), cudaMemcpyHostToDevice,stream), "d_range copy");
        checkCuda(cudaMemcpyAsync(d_rows, rows.data(), numMatrices * sizeof(int), cudaMemcpyHostToDevice,stream), "d_rows copy");
        checkCuda(cudaMemcpyAsync(d_cols, cols.data(), numMatrices * sizeof(int), cudaMemcpyHostToDevice,stream), "d_cols copy");
        ...
        checkCuda(cudaMemcpyAsync(d_prefix_blocks, prefix_blocks.data(), (numMatrices + 1) * sizeof(int), cudaMemcpyHostToDevice,stream),"d_prefix_blocks copy");
		checkCuda(cudaStreamSynchronize(stream), "stream sync before kernels");
		int sharedMemSize = 0;
        countFreqKernel<<<prefix_blocks[numMatrices], 1024, sharedMemSize, stream>>>(d_input, d_range, d_rows, d_cols, d_prefix_blocks, numMatrices, prefix_global, max_padded_size, 1024);
        checkCuda(cudaGetLastError(), "countFreqKernel launch");
		checkCuda(cudaStreamSynchronize(stream), "countFreqKernel stream sync");
        //checkCuda(cudaDeviceSynchronize(), "countFreqKernel sync");
		int writeBackSharedMem = 0;
        writeBackKernel<<<numMatrices, 1024, writeBackSharedMem, stream>>>(d_input, d_range, d_rows, d_cols, numMatrices, prefix_global, max_padded_size);
        checkCuda(cudaGetLastError(), "writeBackKernel launch");
        //checkCuda(cudaDeviceSynchronize(), "writeBackKernel sync");
		checkCuda(cudaStreamSynchronize(stream), "writeBackKernel stream sync");
