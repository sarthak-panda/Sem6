...
BEGIN
    ...
    IF NEW.match_id !~ '^[a-zA-Z0-9]+[0-9]{3}$' THEN
        RAISE EXCEPTION 'sequence of match id violated1';
    END IF;
    ...
    IF NEW.season_id <> season_part THEN
        RAISE EXCEPTION 'sequence of match id violated2';
    END IF;
    IF seq_part = 1 THEN
        IF EXISTS (SELECT 1 FROM public.match WHERE match_id = NEW.match_id) THEN
            RAISE EXCEPTION 'sequence of match id violated3';
        END IF;
    ELSE
        ...
        IF NOT EXISTS (SELECT 1 FROM public.match WHERE match_id = prev_match_id) THEN
            RAISE EXCEPTION 'sequence of match id violated4';
        END IF;
        IF EXISTS (SELECT 1 FROM public.match WHERE match_id = NEW.match_id) THEN
            RAISE EXCEPTION 'sequence of match id violated5';
        END IF;
    END IF;
    RETURN NEW;
END;
