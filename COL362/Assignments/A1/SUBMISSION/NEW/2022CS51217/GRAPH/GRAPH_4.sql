...
, overall_visited as (
	SELECT distinct unnest(visited_nodes) as node
	FROM shortest_path
)
SELECT (count(*)-1) as count --'-1' for the start node itself
from overall_visited